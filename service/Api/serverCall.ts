...
export const BASE_URL: string | undefined =
  "https://legaldocs.unibyts.com/api/";
...
function postCallTicketing({
  ...
  return axios
    .post(
      `${customUrl || `${BASE_URL}${url}`}${customUrl ? "?forData=true" : ""}`,
      data,
      requestOptions
    )
    .then((response) => response)
...
function getCallWithId({
  ...
  return axios
    .get(
      customUrl ? `${customUrl}${url}/${id}` : `${BASE_URL}${url}/${id}`,
      requestOptions
    )
    .then((response) => response)
    .catch((error) => {
